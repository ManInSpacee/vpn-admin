...
## 13. Что сделано (по состоянию на 2026-04-05)
...
- [x] PUT /me/password -- смена пароля (требует старый пароль, защита от угона сессии)
- [x] GET /me -- возвращает email, createdAt, plan { active, expiresAt } или null
- [x] Названия серверов в VLESS ссылках -- из Server.name через encodeURIComponent (после #)
- [x] expiryTime: 0 при создании клиента в x-ui3 -- срок не отображается в remarks
- [x] email клиента в x-ui3 -- slot_N вместо реального email (не утекает email, нет лишнего в названии)
- [x] POST /clients (старый admin API) -- задепрекейчен, возвращает 410
- [x] .gitignore -- добавлены .claude, CONTEXT.md, prisma/seed.ts
- [x] Схема обновлена (Витя): UUID вместо Int для id, поле name у VpnProfile, поле active у Plan

## 14. Что осталось

### Бэкенд (ближайшее)
- [ ] Интеграция FreeKassa (платёжная система) -- ждём данные от Льва
- [ ] Управление серверами через админку (POST/PUT /admin/servers) -- сейчас только через seed
- [ ] Фронт ЛК
...
7. **email клиента в x-ui3** -- формат `slot_N` (не реальный email). Уникальность в рамках инбаунда обеспечивается slotNumber.
8. **expiryTime: 0** -- клиенты в x-ui3 бессрочные. Срок подписки контролируется только нашим бэкендом (VpnProfile.expiresAt). Cron для деактивации истёкших профилей ещё не реализован.
9. **Названия серверов** -- задаются через Server.name в БД. Сейчас меняются только вручную (Prisma Studio или seed). Эндпоинта PUT /admin/servers нет.
10. **Аватар подписки** -- стандарт subscription URL поддерживает заголовок profile-icon. Не реализовано, можно добавить в sub.routes.ts.
